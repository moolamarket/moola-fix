 ...
 * @author Aave
 **/
contract LendingPoolWithGracePeriod is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  // Added for disabling all other actions except deposit and repay during the 48 grace period
  modifier disabledDuringGracePeriod() {
    revert('The protocol is on grace period and it only allows deposit and repay.');
    _;
  }
  ...
  function withdraw(
    ...
    uint256 amount,
    address to
  ) external override whenNotPaused disabledDuringGracePeriod returns (uint256) {
    DataTypes.ReserveData storage reserve = _reserves[asset];
  ...
  function borrow(
    ...
    uint16 referralCode,
    address onBehalfOf
  ) external override whenNotPaused disabledDuringGracePeriod {
    DataTypes.ReserveData storage reserve = _reserves[asset];
